...
public class AboutFragment extends PreferenceFragment implements Preference.OnPreferenceClickListener {
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        mVersion.setSummary(getActivity().getString(R.string.version_name));
    }


    @Override public boolean onPreferenceClick(Preference preference) {
        if (mVersion == preference) {
            new AlertDialog.Builder(getActivity()).setTitle("校园互助系统的完成离不开开源项目的支持，向以下致谢：")
                                                  .setMessage("MaterialViewPager,Google Support Design,Butterknife," +
                                                          "circleimageview以及给力的LeanCloud后台服务")
            ...
            sharingIntent.putExtra(Intent.EXTRA_TEXT, getString(R.string.my_about_page));
            ...
            Uri uri = Uri.parse(getString(R.string.mBolg));   //指定网址
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_VIEW);           //指定Action
            intent.setData(uri);                            //设置Uri
            getActivity().startActivity(intent);        //启动Activity
